...
<s:elseif
	test="actionName != null && (actionName.equals('gestioneClienti_viewEvasionePrenotazioniClientiEdicola.action'))">
	<s:set name="an"
		value="%{'gestioneClienti_viewEvasionePrenotazioniClientiEdicola.action'}" />
</s:elseif>
<s:else>
	<s:set name="an"
		value="%{'viewRifornimenti_showRichiesteRifornimenti.action'}" />
</s:else>
<s:set name="ap" value="#context['struts.actionMapping'].namespace" />
<s:if test='#context["struts.actionMapping"].namespace == "/"'>
	<s:set name="ap" value="" />
</s:if>
<form id="RichiestaRifornimentoForm"
	action="${pageContext.request.contextPath}${ap}/${an}"
	onsubmit="return (ray.ajax())" method="POST">
	<div style="width: 100%; text-align: center;">
		...
		<fieldset class="filterBolla"
			style="margin-top: 0px; top: 0px; width: 90%; text-align: center">
			<legend style="font-size: 100%">
				<s:text name="filterTitle" />
			</legend>
			<s:if
				test="actionName1 == null || !actionName1.contains('viewClienteEdicola_viewPrenotazioniClientiEdicola.action')">
				<div class="required">
					...
					<div style="float: left; width: 300px; text-align: left">
						...
						&nbsp;&nbsp;
						<s:textfield name="dataDa" id="dataDa" cssClass="required"
							cssStyle="width:80px;" disabled="false" />
						&nbsp;&nbsp;
						<s:text name="igeriv.a" />
						&nbsp;&nbsp;
						<s:textfield name="dataA" id="dataA" cssClass="required"
							cssStyle="width:80px;" disabled="false" />
					</div>
				</div>
			</s:if>
			...
			<s:if
				test="%{#request.tipoRifornimento == 'clienteEdicola' || #request.tipoRifornimento == 'edicola'}">
				<div class="required"
					style="align: center; text-align: center; width: 550px">
					<div style="float: left; width: 200px;">&nbsp;</div>
					<div style="float: left; width: 100px;">
					...
					<div style="float: left; width: 200px;">
						<s:if
							...
							<s:select label="" name="stato" id="stato" listKey="key"
								listValue="value"
								list="%{#application['statiPrenotazioniPubblicazioniClient']}"
								emptyOption="true" cssStyle="width: 250px" />
						</s:if>
						<s:else>
							<s:select label="" name="stato" id="stato" listKey="key"
								listValue="value"
								list="%{#application['statiPrenotazioniPubblicazioni']}"
								emptyOption="true" cssStyle="width: 250px" />
						</s:else>
					</div>
				</div>
			...
			<div class="required" style="align: center; text-align: center;">
				<div
					style="float: left; width: 100%; align: center; text-align: center;">
