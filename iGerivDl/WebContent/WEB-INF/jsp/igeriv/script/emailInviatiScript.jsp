...
// 		});	
		...
		$('#MessaggiTab_table tbody tr').each(function() {				
			...
			if (typeof($(this).attr('an')) !== 'undefined' && $(this).attr('an') != '') {				
				str += '<a href="${pageContext.request.contextPath}/attachment.action?codRivendita=<s:text name="%{#request.codRivendita}"/>&fileName=' + $(this).attr('an') + '" target="new"><img src="/app_img/attachment.png" border="0px" style="border-style: none" title="<s:text name="igeriv.allegato"/>" alt="<s:text name="igeriv.allegato"/>" /></a>';	
			}   
			str += '</span>';			
			child1.html(str);	
		});	
