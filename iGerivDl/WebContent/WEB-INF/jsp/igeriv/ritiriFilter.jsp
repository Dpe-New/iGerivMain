...
<s:form id="RitiriFilterForm" action="ritiri_show.action" onsubmit="return (ray.ajax())" method="POST" theme="simple" validate="false">
	<div style="width:100%; text-align:center;">		
		...
		<fieldset class="filterBolla" style="width:610px;"><legend style="font-size:100%"><s:text name="tableTitle"/></legend>
			<div class="required" style="width:550px;">
				<div style="float:left; width:200px;"><s:text name="igeriv.data.ritiro" /></div>
			...
			<s:if test="authUser.tipoUtente == 1">
				<div class="required" style="align:center; width:550px">
					<div style="float:left; width:225px;"><s:text name="username.cliente" /></div>
						...
						<select name="codCliente" id="codCliente" style="width:65%;">
							...
							<optgroup label="<s:text name="igeriv.con.estratto.conto"/>">
								<s:iterator value="listClientiConEC" var="position">
									<option value="<s:property value="#position.codCliente"/>"><s:property value="#position.nomeCognomeEscaped"/></option>
								</s:iterator>
							</optgroup>
			...
			<div style="align:center; text-align:center; width:100%; height:50px;">
				<s:if test="authUser.tipoUtente == 1">
					<div style="float:left; width:25%; align:center; text-align:center;">&nbsp;</div>
				</s:if>
				<div style="float:left; width:<s:if test="authUser.tipoUtente == 1">58%</s:if><s:else>610px</s:else>; align:center; text-align:center;">					
				...
				<s:if test="authUser.tipoUtente == 1">
					<div style="float:left; width:10%; align:center; text-align:center;">		
						<img id="ritiriCanc" src="/app_img/<s:if test="hasRitiriCancellati eq true">Bin_full.png</s:if><s:else>Bin_empty.png</s:else>" style="cursor:pointer" alt="<s:text name="igeriv.ritiri.cancellati"/>" border="0" title="<s:text name="igeriv.ritiri.cancellati"/>"/>
					</div>
				</s:if>
			</div>			
		</fieldset>
