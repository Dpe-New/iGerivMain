...
public class AutomcompleteAction<T extends BaseVo> extends RestrictedAccessBaseAction implements State, 
	...
	public String giriTipo() {
		...
		List<GiroTipoDto> listGiroTipo = messaggiService.getGiriTipo(getAuthUser().getCodFiegDl(), getAuthUser().getId(), codice, desc);
		localita = new ArrayList<Map<String, String>>();
	...
	public String zoneTipo() {
		...
		List<GiroTipoDto> listGiroTipo = messaggiService.getZoneTipo(getAuthUser().getCodFiegDl(), getAuthUser().getId(), codice, desc);
		localita = new ArrayList<Map<String, String>>();
		for (GiroTipoDto vo : listGiroTipo) {
			Map<String,String> m = new HashMap<String, String>();	
			m.put("id", "" + vo.getIdGiroTipo());
	...
	public String getProdottoNonEditorialeInBolla() {
		List<ProdottiNonEditorialiPrezziAcquistoVo> listProdotti = prodottiService.getProdottiNonEditorialiPrezziAcquisto(getAuthUser().getCodEdicolaMaster(), codFornitore, soloProdottiForniti, term, true);
		localita = new ArrayList<Map<String, String>>();
		for (ProdottiNonEditorialiPrezziAcquistoVo vo : listProdotti) {
			...
			m.put("percentualeResaSuDistribuito", (prodotto.getPercentualeResaSuDistribuito() != null ? prodotto.getPercentualeResaSuDistribuito().toString() : ""));
