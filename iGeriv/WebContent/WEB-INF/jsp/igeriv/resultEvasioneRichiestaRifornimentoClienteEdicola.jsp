...
<div id="contentDiv" style="margin-top: 30px">
	...
	<s:if
		...
		<s:form id="EvasioneClienteEdicolaForm"
			action="gestioneClienti_showResultEvasionePrenotazioniClientiEdicola.action"
			method="POST" theme="simple" validate="false"
			onsubmit="return (ray.ajax())">
			<div
				style="width: 100%; text-align: center; color: #444444; font-weight: bold; font-size: 15px; vertical-align: middle;">
				<s:text name='igeriv.evasione.conclusa' />
			</div>
			<div style="width: 100%; text-align: center">
				<input type="button"
					value="<s:text name='igeriv.torna.evasione.clienti'/>" name="torna"
					class="tableFields" style="width: 180px; text-align: center"
					onclick="javascript: window.location = '${pageContext.request.contextPath}/gestioneClienti_showClientiEvasionePrenotazioni.action'" />
			</div>
			<div id="mainDiv" style="width: 100%">
				<dpe:table tableId="RichiesteRifornimentiClienteEdicola"
					...
					style="height:${tableHeight}" rowsDisplayed="1000"
					...
					<dpe:exportPdf fileName="evasione_ordini.pdf"
						...
						marginTopInches="1.2" />
					<ec:exportXls fileName="evasione_ordini.xls"
						tooltip="plg.esporta.excel" />
					<dpe:row style="height:30px" interceptor="marker"
						href="#?idtn=pk.idtn&codCliente=pk.codCliente">
						<ec:column property="nomeCognomeCliente" width="15%"
						...
						<dpe:column property="numeroCopertina" width="7%"
							...
							filterable="false" sortable="false" />
						<dpe:column property="quantitaConsegnare" maxlength="3" size="4"
							width="5%" title="igeriv.richiesta.rifornimenti.quantita.evasa"
							filterable="false" styleClass="extremeTableFieldsSmaller"
							sortable="false" style="text-align:center" />
						<dpe:column property="dataUscita" cell="date" format="dd/MM/yyyy"
							width="7%" title="igeriv.data.uscita"
							styleClass="extremeTableFieldsLarger" filterable="false"
							sortable="false" style="text-align:center"
							headerStyle="text-align:center" />
						<dpe:column property="prezzoCopertina" cell="currency"
							format="###,###,##0.00" style="text-align:right"
							headerStyle="text-align:right" width="8%"
							title="igeriv.prezzo.lordo" filterable="false" sortable="false" />
						<dpe:column property="importoConsegnare" cell="currency"
							format="###,###,##0.00" style="text-align:right"
							headerStyle="text-align:right" width="8%" title="igeriv.importo"
							filterable="false" sortable="false" />
			...
			<div style="width: 100%; text-align: center">
				<input type="button"
					...
					onclick="javascript: window.location = '${pageContext.request.contextPath}/gestioneClienti_showClientiEvasionePrenotazioni.action'" />
				<s:if
					test="idCliente != null && idCliente != '' && #session.mapParamsEdicola.paramEdicola20.paramValue eq 'true'">
			&nbsp;&nbsp;<input type="button"
						value="<s:text name='igeriv.aggiungi.altre.vendite'/>"
						name="torna" id="torna" class="tableFields"
						style="width: 180px; text-align: center"
						onclick="javascript: creaContoCliente()" />
				</s:if>
			</div>
			<s:hidden id="qtaDaEvadere" name="qtaDaEvadere" />
			<s:hidden id="idConto" name="idConto" />
		</s:form>
		<form id="ContoClienteForm"
			action="${pageContext.request.contextPath}/vendite_showConto.action"
			method="POST" target="_top">
			<s:hidden name="idCliente" id="idCliente" />
			<s:hidden name="idConto" id="idConto1" />
			<s:hidden name="contoCliente" id="contoCliente" />
		</form>
		<form id="ReportVenditeForm"
			action="${pageContext.request.contextPath}/report_reportVenditePdf.action"
			method="POST"></form>
		<div id="one"></div>
	</s:if>
	<s:else>
