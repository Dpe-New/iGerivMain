 ...
 *
 */
@Component("TokenRestService")
class TokenRestService  extends BaseRestService<String, VenditeParamDto> implements RestService<String, VenditeParamDto> {
	...
	@Autowired
	TokenRestService(AccountService accountService, PasswordEncoder passwordEncoder, ReflectionSaltSource saltSource, @Value("${igeriv.rtae.rest.url}") String rtaeRestUrl, @Value("${igeriv.env.deploy.name}") String env) {
