...
/**
 * Crea le righe da esportare per le vendite e aggiunge le righe
...
@Component("InforivVenditeExporter")
public class InforivVenditeExporter extends InforivBaseExporter implements InforivExporter {
	@Autowired
	...
	public void exportData(String tipoRecord, Timestamp now, Integer codFiegDl, Map<Integer, File> mapExportFiles, Map<Integer, Map<String, String>> mapFtpParams, FixedFormatManager manager,File dirOutputLocalDL) {
		List<InforivVenditeDto> bolleTrasmesseDl = exportBo.getVenditeDaTrasmettereDl(codFiegDl);
		Group<InforivVenditeDto> groupBolleTrasmesse = group(bolleTrasmesseDl, by(on(InforivVenditeDto.class).getCodEdicola()));
		for (Group<InforivVenditeDto> subgroup : groupBolleTrasmesse.subgroups()) {
			List list = subgroup.findAll();
			exportLines(tipoRecord, now, codFiegDl, mapExportFiles, mapFtpParams, manager, list,dirOutputLocalDL);
		}
		exportBo.updateVenditeTrasmesse(codFiegDl);
	}
