...
<s:if test="%{#request.ordini}">
	<s:form id="PrenotazioneClienteForm"
		...
		<div id="mainDiv" style="width: 100%">
			<dpe:table tableId="PrenotazioneCliente_table" items="ordini"
				var="ordini"
				action="clientiEdicola_showPrenotazioniClienteEdicola.action"
				imagePath="/app_img/table/*.gif" style="height:100px;"
				rowsDisplayed="1000" view="buttonsOnBottom" locale="${localeString}"
				state="it.dpe.igeriv.web.actions.ClientiEdicolaAction"
				styleClass="extremeTableFieldsLarger" form="PrenotazioneClienteForm"
				...
				<dpe:exportPdf fileName="prenotazioni_clienti.pdf"
					...
					isLandscape="false" />
				<ec:exportXls fileName="prenotazioni_clienti.xls"
				...
				<ec:row>
					...
					<dpe:column property="quantitaRichiesta" validateIsNumeric="true"
						maxlength="3" size="4" width="8%"
						title="igeriv.richiesta.rifornimenti.quantita.richiesta"
						cell="textDifferenza" sessionVarName="quantitaRifornimento"
			...
			<div class="tableFields" id="prenotazioneDiv"
				...
				<div style="float: left;">
					<s:text name="igeriv.prenotazione.fissa" />
					&nbsp;&nbsp;&nbsp;
					<s:textfield name="richiestaFissaClienteVo.quantitaRichiesta"
						id="richiestaFissaClienteVo.quantitaRichiesta"
						cssClass="extremeTableFields" cssStyle="width:50px;"
						disabled="%{prenotazioneDisabled}" />
				</div>
				<div style="float: left;">
					<s:text name="igeriv.numero.volte" />
					&nbsp;&nbsp;&nbsp;
					<s:select name="richiestaFissaClienteVo.numeroVolte"
						id="richiestaFissaClienteVo.numeroVolte"
						list="%{#application['listNumberVo1to100']}" listKey="keyInt"
						listValue="value" cssClass="extremeTableFields"
						cssStyle="width:80px;" disabled="%{prenotazioneDisabled}" />
				</div>
				<div style="float: left;">
					<s:text name="igeriv.data.richiesta" />
					&nbsp;&nbsp;&nbsp;
					<s:textfield name="richiestaFissaClienteVo.dataRichiesta"
						id="richiestaFissaClienteVo.dataRichiesta"
						cssClass="extremeTableFields" cssStyle="width:80px;">
						<s:param name="value">
							<s:date name="richiestaFissaClienteVo.dataRichiesta"
								format="dd/MM/yyyy" />
						</s:param>
					</s:textfield>
				</div>
			...
			<div style="width: 450px; margin-top: 50px; margin-left: 360px;">
				<div
					style="float: left; width: 200px; height: 50px; margin-top: 10px;">
					<input type="button" value="<s:text name='igeriv.memorizza'/>"
						name="igeriv.memorizza" id="memorizza" class="tableFields"
						style="width: 100px; text-align: center"
						onclick="javascript: setTimeout(function() { return (validateFields('PrenotazioneClienteForm') && setFormAction('PrenotazioneClienteForm','clientiEdicola_savePrenotazioniClienteEdicola.action', '', 'messageDiv'));},10);" />
				</div>
			</div>
		</div>
	</s:form>
