...
<%-- 				<strong><s:text	name="%{#request.infoordine.numeroOrdine}" /></strong> --%>
		...
		<dpe:table tableId="LibriScolasticiOrdineTab" items="dettaglioordine"
			...
			<dpe:row style="height:30px; cursor:pointer">
				...
				<ec:column property="seqordine" title="igeriv.op.grid" filterable="false" sortable="false" width="4%" style="text-align:center" viewsDenied="pdf,xls">
					<img src="/app_img/del_book.png" width="30px" height="30px" border="0" style="border-style: none" onclick="deleteLibroDalCarrello(${pageScope.dettaglioordine.sku});" />
				</ec:column>
			</dpe:row>
		</dpe:table>

		<table id="infoFooterPopUpDettaglioOrdine" width="810px"
			cellspacing="3">
			<tr>
				<td colspan="2"><input type="button"
					value="<s:text name='dpe.ordine.conferma.intero.ordine'/>"
					name="confermaOrdine" id="confermaOrdine" class="tableFields"
					style="width: 170px; height: 35px; text-align: center"
					 /></td>
				<td style="text-align: right;"><s:text
						name="dpe.ordine.totale.ordine" /></td>
				<td><strong><s:text
							name="%{#request.sumTotPrezzoLibri}" /></strong></td>
			</tr>
		</table>
	</div>
