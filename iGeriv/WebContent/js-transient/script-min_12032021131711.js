$(window).ready(function() {
	...
	var $lastBread = $("#breadcrumb").find("a:last");
	if ($lastBread.length > 0) {
		var href = $lastBread.attr("href");
		var hrefChilds = href.substring(0, href.indexOf(".action") + 7);
		hrefChilds = hrefChilds.substring(hrefChilds.lastIndexOf("/") + 1);
		var href1 = href.substring(0, href.indexOf(".action"));
		var href2 = href1.substring(href1.lastIndexOf("/") + 1);
		var href3 = href2.substring(0, href2.indexOf("_") + 1);
		var $menuList = $("#mydroplinemenu");
		var $menu = $menuList.find("a[id*='" + href3 + "']");
		$menu = $menu.length == 0 ? $menuList.find("a[childs*='" + hrefChilds + "']") : $menu;
		if ($menu.length > 0) {
			$menu.parents("ul").each(function() {
				var $span = $(this).prev("span.dir");
				if ($span.text().length > 0) {
					$span.find("a:first").css({"color":"#FA7204"});
				}
			});
		}
	}
	...
	// MESSAGGI DALLA DPE - NOVITA' (SCRITTA FISSA IN ALTO)
	if (hasMessaggiDpe) {
		var div = '<a href="javascript: openDiv(\'popup_name\', 950, 500, \'' + appContext + namespace + '/msgDpe_viewActiveMessages.action?abilitati=true\');" style="text-decoration:none; text-align:top; margin-top:0; color:#336699; font-family:century; font-style: italic; font-weight:bold; font-size:120%;">' + msgNovita + '</a>';
	...
	// MESSAGGI DL PRIORITA' MEDIA (SCRITTA LAMPEGGIANTE IN ALTO)
	...
	// MESSAGGI A TENDINA
	if (hasMessaggiPrioritaMassima  && categoriaMessaggio!='3') {
		// AVVISO MESSAGGIO DL PRIORITA' MASSIMA
		showAlertMessaggioMassimaPriorita(messaggioNonLetto, priorita);
	} else if (hasMessaggiPrioritaAlta && categoriaMessaggio!='3') {
		// AVVISO MESSAGGIO DL PRIORITA' ALTA
		showAlertMessaggioAltaPriorita(messaggioNonLetto, priorita);
	} else if (hasLivellamenti && (listIdLivellamenti != '' && listIdLivellamenti.length > 0)) {
		// AVVISO PRESENZA RICHIESTE RIFORNIMENTO LIVELLAMENTI
		showPromptLivellamenti(listIdLivellamenti);
	} else if (hasLivellamenti && (listIdLivellamentiAccettati != '' && listIdLivellamentiAccettati.length > 0) && categoriaMessaggio=='3') {
		// AVVISO RISPOSTA A UNA RICHIESTA RIFORNIMENTO LIVELLAMENTI
		//02/04/2015 sostituito con messaggio normale
		showPromptAccettazioneLivellamenti(messaggioNonLetto, priorita);
	}  else if (hasMessaggiPrioritaAlta && categoriaMessaggio=='3') {
		showPromptAccettazioneLivellamenti(messaggioNonLetto, priorita);
	}
	...
	if (hasArgomentiAiuto) {		
		...
		if (actionName.indexOf('vendite_') != -1) {
			$("#help").remove();
			$("#content1").prepend('<div id="help" style="position:absolute; cursor: pointer; margin-top:-40px; margin-left:20px;"></div>');
		}
		$("#help").html('<img id="questionHelp" title="' + helpArgumentsTitle + '" src="/app_img/question_48x48' + style_suff + '.png"/>');
	...
	// NOTIFICA PRESENZA NUOVI ORDINI CLIENTI (ICONA LAMPEGGIANTE IN ALTO)
	...
	$("#rtaeImg, #logoutImg, #igerivImg, #dlDivImg, #questionHelp, #forum, #blog, #fb, #finalizza, #newsDvtdImg, #downloadClientVendite").tooltip({
		delay: 0,  
	    showURL: false
	}); 
...
function showActionNotAllowedMsg() {
	$.alerts.dialogClass = "style_1";
	jAlert(msgActionNotAllowed, attenzioneMsg.toUpperCase(), function() {
		$.alerts.dialogClass = null;
	});
	return false;
}
