...
//	});
	...
	var prev_val = fornVal;
	$("#fornitoriEdit").focus(function() {
	    prev_val = $(this).val();
	}).change(function() {
		$(this).unbind('focus');
		if ($(this).val() == codDl) {
	...
	if (disabled) {
		$("#BollaCaricoEditForm input:not(input[type=hidden])").each(function() {
			$(this).attr("disabled", true);
		});	
		$("#BollaCaricoEditForm select").each(function() {
			$(this).attr("disabled", true);
		});	
		$("#addRow, #deleteRow, #addFornitore").removeAttr("onclick");
	} else {
		if (isNew && codFornitore != null) {
...
function addTableRow() {
	...
	tabRow += '<td style="text-align: left;" width="25%; white-space: nowrap;"><input name="dettagli[' + index + '].prodotto.barcode" id="barcode' + index + '" value="" class="extremeTableFieldsSmaller" style="font-size: 100%; text-align: left; vertical-align:middle" size="20" maxlength="20" type="text">&nbsp;<img src="/app_img/product.gif" width="18px" height="18px" id="product" alt=' + msgAggiungiProdotto + ' border="0" title="' + msgAggiungiProdotto + '" style="cursor:pointer; vertical-align:middle" onclick="javascript: addProduct();"/></td>';
	tabRow += '<td width="35%" style="display:block; white-space:nowrap;"></td>';
...
function attachEventsToRow($row, index) {
	...
	$prodField.autocomplete({	
		...
		source: function(request, response) {	
			...
			dojo.xhrGet({
				url: appContext + '/pubblicazioniRpc_getPneBarcodeOrDescrizione.action?barcode=' + $val + "&findProdottiSenzaFornitore=true",
				delay: 100,
				preventCache: true,
				handleAs: "json",				
				headers: { "Content-Type": "application/json; charset=uft-8"}, 	
		...
		$('#BollaCaricoEditTab_table tbody tr').each(function() {
			...
			$(this).find("input[name*='dettagli'], select[name*='dettagli']").each(function() {
				...
				if (fieldId.indexOf("prezzo") != -1 && $(this).val() == '') {
					...
					unBlockUI();
					throw fieldId;
				} else if (fieldId.indexOf("prezzo") != -1 && Number($(this).val()) > 1000) {
					if (showAlerts) {
						$.alerts.dialogClass = "style_1";
						jAlert(numeroNonValido, msgAttenzione, function() {
							$.alerts.dialogClass = null;
