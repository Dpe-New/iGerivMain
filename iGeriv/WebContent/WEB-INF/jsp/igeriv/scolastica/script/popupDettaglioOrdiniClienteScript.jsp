...
<script type="text/javascript">
	...
	function viewTracking(numOrdineTxt) {
		...
		if (numOrdineTxt==null) {
			numOrdineTxt="0";
		}
		var pattern = /^\d+$/;
	    var isNum =  pattern.test(numOrdineTxt);
		if (!isNum) {
			jAlert('<s:text name="Tracking non disponibile"/>', attenzioneMsg.toUpperCase());
			return;
		}
		...
		jConfirm('<s:text name="dpe.ordine.msg.view.tracking.ordine"/>', attenzioneMsg.toUpperCase(), function(r) {
			 ...
			 if (r) { 
			    	dojo.xhrGet({
			    		...
			    		//url: "${pageContext.request.contextPath}/libriScolasticiTracking_showFilterTracking.action",	
			    		url: "${pageContext.request.contextPath}/libriScolasticiTracking_showTrackingClesp.action",	
						preventCache: true,
						handleAs: "json",				
						...
						handle: function(data,args) {
							...
							<s:url action="libriScolasticiTracking_showTrackingClesp.action" var="urlTag"/>
