	...
	public String getHtmlDisplay(TableModel model, Column column) {
		...
		try {
			...
			if (column.getValue() != null) {
				...
				} else if (!numberFormat.equals("") && org.apache.commons.lang.math.NumberUtils.isNumber(column.getValue().toString())) {
					columnValue = NumberUtils.formatNumber((Number) column.getValue());
				} else {
			...
			// TODO Refactoring
			// Andrebbe implementata una org.extremecomponents.table.cell.Cell per ognuno di questi casi e dichiararla nel file extremetable-dpe.properties
			...
			else if (bean instanceof RichiestaRifornimentoDto) {
				Timestamp dataUscita = ((RichiestaRifornimentoDto)bean).getDataUscita();
			...
			if (textDifferenze != null
				...
				if (!fieldEnabled || !isTextFieldEnabled(bean, enabledConditionFieldsArr, enabledConditionValuesArr)) {
				...
				if (!fieldEnabled || !isTextFieldEnabled(bean, enabledConditionFieldsArr, enabledConditionValuesArr) && (permetteInserimentoRichiesteRifornimentoFuture == null || !permetteInserimentoRichiesteRifornimentoFuture)) {
					columnBuilder.getHtmlBuilder().disabled();
				} 
				columnBuilder.getHtmlBuilder().xclose();
				if (hasHiddenPkField) {
