...
<script>	
	...
	function validateExtraFields(showMsg) {
		...
		$('#EvasioneClienteEdicolaForm input:text[name^=evaso]').each(function() {
			...
			var $currField = $(this);
			var $qtaEvasa = $currField.val();
			if ($qtaEvasa >= 0) {
				hasValues = true;
			}
			if ($qtaEvasa > 0 && $qtaEvasa.trim().length > 0 && Number($qtaEvasa.trim()) > Number(qtaDaEvadere)) {
				if (showMsg) {
					$.alerts.dialogClass = "style_1";
		...
		if (!hasValues) {
			if (showMsg) {
				$.alerts.dialogClass = "style_1";
				jAlert('<s:text name="msg.nessun.valore"/>', '<s:text name="msg.alert.attenzione"/>', function() {
					$.alerts.dialogClass = null;
				});
			}
			ret = false;
				...
				if (errMsg != '') {
					...
					if ($('#RichiesteRifornimentiClienteEdicola_table', $doc).length > 0) {
						...
						if (getBrowser().indexOf("FIREFOX") == -1) {
							var colIndex = <s:if test="spunte != null && !spunte.isEmpty()">8;</s:if><s:else>7;</s:else>
							var colIndex1 = <s:if test="spunte != null && !spunte.isEmpty()">10;</s:if><s:else>9;</s:else>
							$("#RichiesteRifornimentiClienteEdicola_table tr td:nth-child(" + colIndex + ")").attr('width', '8%');
							$('tr td:nth-child(' + colIndex1 + '), tr th:nth-child(' + colIndex1 + ')', $("#RichiesteRifornimentiClienteEdicola_table")).remove();			
						}
					} else {
						dojo.byId('contentDiv').innerHTML = '<div style="width:100%; text-align:center; color: #444444; font-weight: bold; font-size: 15px; vertical-align: middle;"><s:text name="igeriv.evasione.conclusa"/></div><div style="width:100%; margin-top:50px; text-align:center"><input type="button" value="<s:text name="igeriv.torna.evasione.clienti"/>" name="torna" id="torna" class="tableFields" style="width:180px; text-align:center" onclick="javascript: window.location = \'${pageContext.request.contextPath}/gestioneClienti_showClientiEvasionePrenotazioni.action\'"/></div>';
					}
				}
			} 
		});
	...
	function printRicevuta() {
		...
		$.each(arrRows, function(index, arr) {
			...
			$.each(arr, function(i, clientArr) {
			    ...
			    var prezzoStr = $row.find("td:nth-child(7)").text().trim();
