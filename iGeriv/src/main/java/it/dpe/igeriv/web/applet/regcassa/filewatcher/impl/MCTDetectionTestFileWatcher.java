...
public class MCTDetectionTestFileWatcher extends FileWatcher {
	...
	public MCTDetectionTestFileWatcher(File file, JSObject window, Timer timer, long millsTimeout, Logger logger) {
		this.file = file;
		this.window = window;
		this.timer = timer;
	...
	protected void onChange(File file, Enum result) {
		logger.fine("Entered MCTDetectionTestFileWatcher.onChange() with params: file=" + file + ", result=" + result);
		try {
		...
		logger.fine("Exiting MCTDetectionTestFileWatcher.onChange()");
	}
