...
(function($) {
	...
	$.alerts = {
		...
		promptReportVenditeRicariche: function(message, title, params, callback) {
			if( title == null ) title = richiestaInfoMsg;
			$.alerts._promptReportVenditeRicariche(title, message, params, 'prompt', function(op, d1, d2) {
				if( callback ) callback(op, d1, d2);
			});
		},
		...
		_promptReportVenditeRicariche: function(title, msg, params, type, callback) {
			...
			$("BODY").append(
				'<div id="popup_container" style="font-size:' + (browserHeight < 700 ? '90' : '100') + '%; color:#808080; width:600px:">' +
			    '<h1 id="popup_title"></h1>' +
			    '<div id="popup_content">' +
			...
			$("#popup_title").text(title);
			$("#popup_content").addClass(type);
			$("#popup_message").html("<span style='color:black; font-weight:bold'>" + msg + "</span>");
			...
			var html = '';
			html += '<center>';
			html += '<div id="opPromptDiv" style="text-align:left; height:60px;">';
			html += '<div style="width:100%; white-space:nowrap; margin-top:3px;"><input type="radio" name="operationPrompt" value="VENDITE_ABBONATO_RIV_PERIODO" checked>&nbsp;&nbsp;' + params.msgVenditeAbbonato + '</div>';
			html += '<div style="width:100%; white-space:nowrap; margin-top:3px;"><input type="radio" name="operationPrompt" value="VENDITE_MINICARD_RIV_PERIODO">&nbsp;&nbsp;' + params.msgVenditeMinicard + '</div>';
			html += '<div style="width:100%; white-space:nowrap; margin-top:3px;"><input type="radio" name="operationPrompt" value="RICARICHE_MINICARD_RIV_PERIODO">&nbsp;&nbsp;' + params.msgRicaricheMinicard + '</div>';
			html += '</div>';
			html += '<div style="height:30px; white-space:nowrap;"><div>' + params.msgDataDa + '&nbsp;&nbsp;<input type="text" value="" size="9" id="dataDaPrompt" name="dataDaPrompt" />&nbsp;&nbsp;' + params.msgDataA + '&nbsp;&nbsp;<input type="text" value="" size="9" id="dataAPrompt" name="dataAPrompt" /></div></div>';
			html += '</center>';
			html += '';
			
			$("#popup_message").append(html).after('<div id="popup_panel"><input type="button" style="width:100px;" value="' + $.alerts.okButton + '" id="popup_ok" /> <input type="button" style="width:100px;" value="' + $.alerts.cancelButton + '" id="popup_cancel" /></div><br>');
			$("#popup_message").find("a").css({"text-decoration":"none"});
			
			$("#popup_ok").click( function() {
				var dtDa = $("#dataDaPrompt").val();
				var dtA = $("#dataAPrompt").val(); 
				var op = $("#opPromptDiv input:radio[name='operationPrompt']:checked").val();
				if (!checkDate(dtDa)) {
					$("#dataDaPrompt").focus();
					return false;
				}
				...
				$.alerts._hide();
				if( callback ) callback( op, dtDa, dtA ); 
				if ($("#" + lastFocusedFieldId).length > 0) {
					$("#" + lastFocusedFieldId).focus();
				}
			});
		...
		_showPromptFile: function(title, msg, params, type, callback) {
			...
			$("BODY").append(
			  '<div id="popup_container" style="font-size:100%; color:#808080; width:450px;">' +
			    '<h1 id="popup_title"></h1>' +
			    '<div id="popup_content">' +
			      '<div id="popup_message"></div>' +
				'</div>' +
			...
			var html = '<form id="upadloadImgForm" enctype="multipart/form-data" name="upadloadImgForm" action="">';
			html += '<br />';
			html += '<div style="width:480px;">';
			html += '<div style="height:30px;"><div id="fileDiv" style="float:left;"><input type="file" name="fileField" id="fileField" style="width:320px"/></div>';
			html += '</form>';
		...
		_show: function(title, msg, value, type, callback, multiline, rowsAllowed, colsAllowed, yesNoOptions, yesSelected) {
			...
			switch( type ) {
				...
				break;
				case 'confirm':
					$("#popup_message").after('<div id="popup_panel"><input type="button" value="' + (hasYesNoOption ? $.alerts.siButton : $.alerts.okButton) + '" id="popup_ok" style="width:100px"/> <input type="button" value="' + (hasYesNoOption ? $.alerts.noButton : $.alerts.cancelButton) + '" id="popup_cancel" style="width:100px"/></div>');
					$("#popup_ok").click( function() {
						$.alerts._hide();
	...
	jPromptReportVenditeRicariche = function(message, title, params, callback) {
		$.alerts.promptReportVenditeRicariche(message, title, params, callback);
	};
