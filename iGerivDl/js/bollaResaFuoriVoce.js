		...
		$("#BollaResaTab_table tbody tr[divParam]").each(function() {
			    ...
			    if (Number(currCpuValue) == Number(formCpuValue)) {
			    	...
			    	if (Number(textResoPk.split("|")[4]) == Number(textResoFuoriVocePk.split("|")[4])) {
			    		...
			    		var giacenza = cpuRow.find("td:nth-child(8)").html().trim();  
		...
		if (!found && arrCpuRows.length > 0) {
			...
			for (i = 0; i < arrCpuRows.length; i++) {
				...
				if (currRowIndex == maxRowIndex) {
					...
					var tipoRichiamo = arrCpuRows[i].find("td:nth-child(14)").html();					
					var img = arrCpuRows[i].find("td:nth-child(15)").html();
					var giacenza = row.find("td:nth-child(6)").html().trim();
					var trTag = getRowHtml(row, cpu, tipoRichiamo, img, resoId, resoVal, false);
			...
			var trTag = getRowHtml(row, formCpuValue, "", img, resoId, resoVal, false);			
			var giacenza = row.find("td:nth-child(6)").html().trim();			
			if (resoVal != null && resoVal != '') {
				hiddenFields += addExtraHiddenFields(resoId, resoVal, giacenza);
			}
...
function getRowHtml(row, cpu, tipRichiamo, img, resoId, resoVal, isContoDeposito) {
	...
	trTag += "<td style='cursor: pointer;' width='16%'>" + row.find("td:nth-child(1)").html() + "</td>";
	trTag += "<td width='12%'>" + row.find("td:nth-child(2)").html() + "</td>";
	trTag += "<td style='text-align: center;' width='4%'>" + row.find("td:nth-child(3)").html() + "</td>";
	trTag += "<td style='text-align: center;' width='5%'>" + row.find("td:nth-child(4)").html() + "</td>";
	trTag += "<td class='extremeTableFieldsSmaller' style='font-size: 120%; font-weight: bold; text-align: right; border-width: 1px medium; border-style: solid none; border-color: red -moz-use-text-color;' width='5%'><input name='resoFuoriVoce[&#39;" + pkVal + "&#39;]' id='" + id + "' value='" + resoVal + "' class='extremeTableFieldsSmaller' isContoDeposito='" + isContoDeposito + "' style='font-size:120%; font-weight:bold; text-align:right; border-color: rgb(153, 153, 153);' size='2' maxlength='3' validateisnumeric='true' isnewfield='true' type='text'><input type='hidden' name='modificatoFV[&#39;" + pkVal + "&#39;]' id='modificatoFV" + pkVal + "' value='false'/></td>";
	trTag += "<td style='text-align: center;' width='5%'>" + row.find("td:nth-child(6)").html() + "</td>";				
	trTag += "<td style='text-align: center;' class='extremeTableFieldsSmaller' style='text-align: right;' width='8%'>" + row.find("td:nth-child(7)").html() + "</td>";
	...
	trTag += "<td style='text-align: center;' width='8%'>" + tipRichiamo + "</td>";
	trTag += "<td style='text-align: center;' width='3%'>" + img + "</td>";
	trTag += "</tr>";	
	return trTag;
}
