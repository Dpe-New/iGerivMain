...
function promptPrivacyClienteEdicola(url) {
	...
	//jPromptPrivacyCliente($.validator.format(privacyMessageLettore, [serverName.replaceAll(".","_")]),'','', $.validator.format(msgAccettoPrivacy, [serverName.replaceAll(".","_")]), params, function(result) {
	jPromptPrivacyCliente($.validator.format(privacyMessageLettore, url_Pdf_Privacy),'','', $.validator.format(msgAccettoPrivacy, url_Pdf_Privacy), params, function(result) {
...
var j;
    ...
    for (i=0; i<sheets_num; i++) {
		if (RichCalendar.is_ie) {
			...
			if (/igeriv_combined.css$/.test(styles[i].href)) {
