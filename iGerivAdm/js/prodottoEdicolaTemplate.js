...
function prepareProdottiGenerici() {
	var $prodotti = $("span[id*='folderTemplate_2_']" );
var $mieiProdotti = $("#prodottiEdicola");
var $mieiProdottiLi = $("#prodottiEdicola li");
var $mieiProdottiItens = $("#prodottiEdicola span.subFolder" );
$prodotti.draggable({
	cancel: "a.ui-icon",
	revert: "invalid",
	helper: "clone",
...
function prepareProdottiEdicola() {
	...
	$("span.level1").contextMenu({menu : 'categoryEditDelete', yTop : -10, xLeft : 0, highlight : true}, function(action, el, pos) {
		editCat(action, el, pos);
	});
	$("span.fileEdicola").contextMenu({menu : 'myMenu', yTop : -10, xLeft : 0, highlight : true}, function(action, el, pos) {
		rightContextMenuWorkProduct(action, el, pos);
	});
	$("#prodottiEdicola span.subFolder").contextMenu({menu : 'subcategoryEditDelete', yTop : -10, xLeft : 0, highlight : true}, function(action, el, pos) {
	...
	function rightContextMenuWorkProduct(action, el, pos) {
		...
		if (action == "delete") {
			...
			openDiv("popup_name", 800, 500, url, '', '', window.parent.document);
		} 
	}
	...
	function editProdotto(action, el, pos) {
		...
		openDiv("popup_name", 800, 540, url, '', '', window.parent.document);
	}
}