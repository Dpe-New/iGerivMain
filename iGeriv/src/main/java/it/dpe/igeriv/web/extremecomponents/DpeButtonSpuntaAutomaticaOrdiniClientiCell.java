...
public class DpeButtonSpuntaAutomaticaOrdiniClientiCell extends HeaderCell {
	...
	protected void buildHeaderHtml(HtmlBuilder html, TableModel model, Column column, String headerClass, String sortImage, String sortOrder) {
        ...
        html.span().style("width:100%; text-align:center").close();
        html.append(column.getTitle());
        html.spanEnd();
        html.span().style("width:100%; text-align:center").close();
        //html.input("button").name(DPEWebContants.DPE_BUTTON_SPUNTA_AUTOMATICA_NAME).id(DPEWebContants.DPE_BUTTON_SPUNTA_AUTOMATICA_NAME).value(model.getMessages().getMessage("igeriv.spunta.auto")).title(model.getMessages().getMessage("igeriv.spunta.auto")).styleClass(DPEWebContants.STYLE_CLASS_TEXT).style("width:40px;align:center;text-align:center;font-size:8.5px").onclick("spuntaAutomaticaOrdiniClienti(this)");
        html.input("button").name(DPEWebContants.DPE_BUTTON_SPUNTA_AUTOMATICA_NAME).id(DPEWebContants.DPE_BUTTON_SPUNTA_AUTOMATICA_NAME).value(model.getMessages().getMessage("igeriv.spunta.auto")).title(model.getMessages().getMessage("igeriv.spunta.auto")).styleClass(DPEWebContants.STYLE_CLASS_TEXT).style("width:40px;align:center;text-align:center;font-size:8.5px").onclick("spuntaAutomaticaOrdiniClienti(this); updateTotale(this);");
        if (column.isSortable()) {
