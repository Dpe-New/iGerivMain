...
public class DpeGiorniValiditaRifornimentoTextCell implements Cell {
	...
	public String getHtmlDisplay(TableModel model, Column column) {
		...
		try {
			...
			String giorniValiditaRichiesta = bean.getGiorniValiditaRichiesteRifornimento() != null ? bean.getGiorniValiditaRichiesteRifornimento().toString() : "7";
			Object user = SecurityContextHolder.getContext().getAuthentication().getPrincipal();
			boolean hasLivellamenti = user != null && ((UserAbbonato) user).getHasLivellamenti();
			if (hasLivellamenti && bean.getLivellamentoEditable() && bean.getGiorniValiditaRichiesteRifornimento() == null) {
				giorniValiditaRichiesta = model.getContext().getApplicationAttribute("giorniScadenzaRichiestaRifornimentoLivellamenti").toString();
			}
			columnBuilder.getHtmlBuilder().input("text").name("giorniValiditaRichiesteRifornimentoMap['" + bean.getPk().toString() + "']").id(bean.getPk().toString()).value(giorniValiditaRichiesta).styleClass("extremeTableFieldsLarger").size("3").maxlength("3");
