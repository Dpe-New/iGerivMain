...
public class EmailValidationFilter extends GenericFilterBean {
	...
	@Autowired
	EmailValidationFilter(EdicoleService edicoleService, AccountService accountService, ClientiService<ClienteEdicolaVo> clientiService, PasswordEncoder passwordEncoder, ReflectionSaltSource saltSource, IGerivLoginSessionConfigurer iGerivLoginSessionConfigurer, @Qualifier("sessionRegistry") SessionRegistry sessionRegistry) {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain) throws ServletException, IOException {
		...
		if (!Strings.isNullOrEmpty(parameter) && !Strings.isNullOrEmpty(userPwd)) {
			...
			if (userType.equals(IGerivConstants.TIPO_UTENTE_EDICOLA)) {
				...
				//Gestione Profili
				viewImg = utenteEdicola.getDlGruppoModuliVo().getGruppoModuli().getViewImageByProfile();
			...
			UserAbbonato user = accountService.buildUserDetails(idUtente, utente);
			...
			if (pwd.equals(userPwd)) {
				if (tipoUtente.equals(IGerivConstants.TIPO_UTENTE_EDICOLA)) {
					...
					clientiService.saveBaseVo(utente);
				}
				...
				int indexOf = sessionRegistry.getAllPrincipals().indexOf(auth.getPrincipal());
				if (indexOf != -1) {
					UserAbbonato uab = (UserAbbonato) sessionRegistry.getAllPrincipals().get(indexOf);
					uab.setEmailValido(true);
				}
				...
				httpServletRequest.getSession().setAttribute(HttpSessionSecurityContextRepository.SPRING_SECURITY_CONTEXT_KEY, SecurityContextHolder.getContext());
 	        	...
 	        	user.setViewImageByProfile(viewImg);
