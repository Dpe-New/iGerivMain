...
public class ResetPwdManagerAction extends RestrictedAccessBaseAction {
	...
	public String resetPwdAdm() throws Exception {
		UserAdminVo user = null;
		String email = null;
		try {
			user = accountService.getUserAdminVo(codEdicola, true);
			email = user.getEmail();
		...
		authList.add(new GrantedAuthorityImpl(user.getGruppoModuliVo().getRoleName()));
		...
		ua.setCodFiegDl(new Integer(codEdicola));
		ua.setCodEdicolaDl(new Integer(codEdicola));
