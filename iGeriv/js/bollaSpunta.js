$(document).ready(function() {
	        ...
	        for (var j = 0, cell; cell = row.cells[j]; j ++) {
	            ...
	            switch (index) {
	                  ...
	                  case 11: {
	                      $(cell).click(function() {
	                          var link = $(this).find("a");
	                          if (link.length > 0 && link.html().trim().length > 0 && link.attr("href").length > 0) {
	                              openOrdiniSpunta(link.attr("href"));
	                          }
	                      });
	                      break;
	                  }
	...
	var $fields = $('#BollaControlloForm input:text[name^=differenze]');
	$fields.change(function() {
		$(this).parent().find("input:hidden[name^=modificato]").first().val("true");
	});
...
function openLayer(obj) {
    ...
    var pk = tr.find("td:nth-child(10)").find("input:text[name^='differenze']:first").attr("id").replace('differenze','');
...
function bollaRivKeyPress(event){
	...
	if ((!event.shiftKey && keycode == '9') || keycode == '40') {	// tab	|| arrow down	
		setTimeout(function() {var id = lastFocusedFieldId.replace(/([|])/g,'\\\$1'); var field1 = $("#" + id); if (field1.val().trim().length > 0) { setDifferenze(field1); } var field = field1.closest("tr").next().find("input:text[name='differenze']").first(); $("#" + id).css('background','#fff'); field.focus(); field.select();}, 100);
	} else if ((event.shiftKey && keycode == '9') || keycode == '38') {	// shift + tab	|| arrow up
		setTimeout(function() {var id = lastFocusedFieldId.replace(/([|])/g,'\\\$1'); var field1 = $("#" + id); if (field1.val().trim().length > 0) { setDifferenze(field1); } var field = field1.closest("tr").prev().find("input:text[name='differenze']").first(); $("#" + id).css('background','#fff'); field.focus(); field.select();}, 100);
	} 
	else if (keycode == 37) { // left arrow
		event.preventDefault();
		
		$("#qta").val("").focus().select();
		//$("#qta").val("1").select();
		//$("#qta").select();
...
function openOrdiniSpunta(href) {
	var popID = 'popup_name'; 
    var popURL = href;
 	var popWidth = 900;
 	var popHeight = 380;
...
function addResultRow(idtn, pk, skipCalc) {
	...
	var qta = $("#qta").val();
	var copieLette = !skip ? Number(copieLettePrec) + Number(qta) : Number(copieLettePrec);
	var differenze = copieLettePrec == 0 ? '' : Number(copieLette) - Number(copieBolla);
...
function hasFieldsToSave() {
	var table = document.getElementById("BollaControlloTab_table");
	if (table != null && $(table).length > 0) {
		for (var i = 0, row; row = table.rows[i]; i ++) {
			if (i > 0) {
				var $row = $(row);
				var modificato = $row.find("input:hidden[name^=modificato]").first().val();
				if (modificato == 'true') {
					return true;
				}
		        for (var j = 0, cell; cell = row.cells[j]; j++) {
...
function resetModifiedFields() {
	$("#BollaControlloForm input:hidden[name^=modificato]").each(function() {
		$(this).val("false");
	});
}

function validateFieldsSpuntaBolla() {
	var table = document.getElementById("BollaControlloTab_table");
	if (table != null && $(table).length > 0) {
...
function memorizzaPrimaDiUscire() {
	$.alerts.dialogClass = "style_1";
	jAlert(msgMemorizzaPrimaDiUscire, attenzioneMsg.toUpperCase(), function() {
		$.alerts.dialogClass = null;
	});
}
...
function saveBollaAuto() {
	setTimeout(
			function() {
				return ( 
						setFormAction('BollaControlloForm','bollaRivenditaSpunta_save.action', '', 
											'messageDiv', false, '', function() { resetModifiedFields(); }, 
											function() {return validateFieldsSpuntaBolla(); unBlockUI();},
											false, function() {unBlockUI();}));
			}
	, 10);
	//unBlockUI();
...
saveAndSendBollaSpunta = function() {
	
	confirmMemorizzaInvia = function() {
		setTimeout(function() {
			jConfirm(memorizzaInviaConfirm, attenzioneMsg, function(r) { 
				if (r) {
					return (validateFieldsSpuntaBolla() && setFormAction('BollaControlloForm','bollaRivenditaSpunta_send.action', '', 'messageDiv', false, '', '', '', true));
				} 
				else {
					unBlockUI();
				};
